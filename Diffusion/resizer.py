...
class Resizer(nn.Module):
    def __init__(self, in_shape, scale_factor=None, output_shape=None, kernel=None, antialiasing=True):
        ...
        # scale_factor=[1, 1, down_N, down_N], output_shape=[BATCH_SIZE, channels, H/down_N, W/down_N]

        # down = Resizer(shape, 1 / modelConfig["down_N"]).to(next(model.parameters()).device)
        # up = Resizer(shape_d, modelConfig["down_N"]).to(next(model.parameters()).device)
        ...
        }.get(kernel) # 使用 .get() 方法根据给定的 kernel 值在字典中查找对应的插值方法和核宽度。

        # Antialiasing is only used when downscaling
        antialiasing *= (np.any(np.array(scale_factor) < 1)) # 当down_N<1时会进行该操作

        # Sort indices of dimensions according to scale of each dimension. since we are going dim by dim this is efficient
        sorted_dims = np.argsort(np.array(scale_factor)) # np.argsort 函数返回了这个数组的排序索引 [2, 3, 0, 1]

        self.sorted_dims = [int(dim) for dim in sorted_dims if scale_factor[dim] != 1]  # 返回不等于1的索引[2,3]
    ...
    def fix_scale_and_size(self, input_shape, output_shape, scale_factor):
        ...
        if scale_factor is not None:
            ...
            if np.isscalar(scale_factor) and len(input_shape) > 1:
                # print("np.isscalar(scale_factor)", np.isscalar(scale_factor)) # True
